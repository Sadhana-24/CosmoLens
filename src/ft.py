...
def data_prep_swift():
    ...
    # Define paths for galaxy classification task
    DATASET_NAME = "MultimodalUniverse/gz10"
    OUTPUT_DIR = "/data/swift_galaxy_data"
    ...
    # Load dataset - if it's large we can sample it
    images_dataset = load_dataset(DATASET_NAME, split="train")
    
    # Sample a portion if needed
    SAMPLE_SIZE = min(10000, len(images_dataset))  # Limit to 10,000 examples or full dataset if smaller
    if SAMPLE_SIZE < len(images_dataset):
        images_dataset = images_dataset.shuffle(seed=42).select(range(SAMPLE_SIZE))
    
    logger.info(f"Loaded {len(images_dataset)} examples")

    def process_single_item(item, idx, pbar=None):
        """Process a single dataset item for galaxy classification"""
        try:
            # Get the image
            img = item['rgb_image']
            
            # Generate a unique image filename
            image_filename = f"galaxy_{idx}.png"
            ...
            # Get the label and convert to class name
            label_id = item["gz10_label"]
            class_name = CLASS_LABELS.get(label_id, "Unknown class")
            
            # Additional metadata that might be useful for training
            redshift = item.get("redshift", 0.0)
            object_id = item.get("object_id", "unknown")
            
            # Create the Swift-compatible format for galaxy classification
            transformed_item = {
                "messages": [
                    {
                        "role": "system", 
                    ...
                    {
                        "role": "user",
                        "content": f"<image> {USER_PROMPT}\n\nAdditional information: Redshift = {redshift}",
                    },
                    {
                        "role": "assistant",
                        "content": f"This image shows a {class_name.lower()}."
                    }
                ],
            ...
            jsonl_path = os.path.join(JSONL_DIR, f"galaxy_{idx}.jsonl")
    ...
    # Process the dataset
    logger.info(f"Processing {len(images_dataset)} items")
    transformed_data = []
    
    # Determine number of workers
    import multiprocessing
    num_workers = min(multiprocessing.cpu_count(), len(images_dataset))
    
    # Use ThreadPoolExecutor
    with tqdm(total=len(images_dataset), desc="Processing with ThreadPoolExecutor") as pbar:
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            futures = {
                executor.submit(
                    ...
                    i, 
                    pbar
                ): i for i in range(len(images_dataset))
            }
...
def train_swift():
    ...
    # Define paths
    DATASET_PATH = "/data/swift_galaxy_data/train.jsonl"
    VAL_DATASET_PATH = "/data/swift_galaxy_data/val.jsonl"
    OUTPUT_DIR = "/runs/galaxy_classification_output"
