...
async def search_pinecone_multiple_namespaces(query_vector: List[float], top_k_per_namespace: int, index_host: str, namespaces: List[str]) -> List[str]:
    """Queries Pinecone index across multiple namespaces and returns relevant context snippets."""
    ...
    try:
        ...
        for namespace in namespaces:
            ...
            try:
                ...
                for match in query_response.matches:
                    ...
                    if metadata:
                        ...
                        if context and isinstance(context, str):  # Ensure context is a non-empty string
                            namespace_contexts.append(f"[{namespace}] {context}")
                        else:
                ...
                print(f"Retrieved {len(namespace_contexts)} contexts from namespace {namespace}")
                all_retrieved_contexts.extend(namespace_contexts)
...
async def perform_rag_query(query: str = Form(...), image: UploadFile = File(None)):
    ...
    try:
        ...
        # Step 2: Search Pinecone across multiple namespaces
        t = time.time()
        ...
        print(f"Step 2: Pinecone search across {len(PINECONE_NAMESPACES)} namespaces took {time.time() - t:.2f}s. Found {len(retrieved_docs)} docs.")
